...
class AddTodoDialog(context: Context, val onAddTodo: (Todo) -> Unit) :
    ...
    private var currentRepeatMode = RemindMode.DAY
    ...
    private fun selectCategory() {
        tvCategory?.text = wpCategory?.data?.get(wpCategory?.currentItemPosition ?: 0).toString()
    }
    ...
    private fun addRepeatTime() {
        if (repeatMode == currentRepeatMode) {
            ...
            val currentRepeatTime = when (repeatMode) {
                RemindMode.DAY -> "每天"
                RemindMode.WEEK -> wpRepeatTime?.data?.get(wpRepeatTime!!.currentItemPosition)
                    ?.toString()

                RemindMode.MONTH -> {
                    val day =
                        wpRepeatTime?.data?.get(wpRepeatTime!!.currentItemPosition)?.toString()
                    if (day != null) "每月${day}日" else null
                }
