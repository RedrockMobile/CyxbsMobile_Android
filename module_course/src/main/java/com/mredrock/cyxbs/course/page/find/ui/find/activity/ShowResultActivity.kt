...
class ShowResultActivity : BaseActivity() {
   ...
   */
  interface PersonData : Serializable
   ...
   * 用于分离学生和老师搜索的结果
   */
  private abstract class PersonHandler(val activity: ShowResultActivity) {
    companion object {
      fun getHandler(data: PersonData, activity: ShowResultActivity): PersonHandler {
        return when (data) {
          ...
          else -> {throw RuntimeException("ShowResultActivity#getHandler 未匹配")}
        }
      }
    }
