package com.mredrock.cyxbs.course.widget.oversize
...
import com.mredrock.cyxbs.course.service.GridWidgetService
import com.mredrock.cyxbs.course.service.GridWidgetService.Companion.getCourse
import com.mredrock.cyxbs.course.utils.changeCourseToWidgetCourse
import com.mredrock.cyxbs.course.utils.getClickIntent
import com.mredrock.cyxbs.course.utils.startOperation
import com.mredrock.cyxbs.course.widget.page.oversized.deleteTitlePref
import java.util.*
...
class OversizedAppWidget : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        when (intent.action) {
            ...
            "btn.start.com" -> {
                val subscribe =
                    getCourse(intent.getIntExtra("POSITION", -1))
                        ?.subscribe {
                            it?.let {
                                startOperation(
                                    changeCourseToWidgetCourse(it))
                                }
                        }
            }
        }
