...
abstract class AbstractCoursePageFragment : BaseFragment(R.layout.course_layout) {
  
  // CourseLayout 保持接口通信，降低耦合度
  protected val mCourseScrollView by R.id.course_scrollView.view<AbstractCourseScrollView>()
  protected val mCourseLayout     by R.id.course_courseLayout.view<AbstractCourseLayout>()
  ...
  @CallSuper
  abstract fun initNoon()
  @CallSuper
  abstract fun initDusk()
  @CallSuper
  abstract fun initTimeLine()
  
  protected inline fun course(block: ICourseLayout.() -> Unit) {
    block.invoke(mCourseLayout)
  }
