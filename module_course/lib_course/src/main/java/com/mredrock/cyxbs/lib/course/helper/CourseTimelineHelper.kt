 ...
 *
 * ### 该类设计：
 * - 对 [AbstractCourseLayout] 增加自定义绘图的监听来实现
...
class CourseTimelineHelper internal constructor(
  ...
  private fun Int.dp2pxF() = course.getContext().resources.displayMetrics.density * this
  ...
  private val mRefreshRunnable = object : Runnable {
    override fun run() {
      course.invalidate()
      course.postDelayed(1000 * 20, this) // 20 秒刷新一次，但记得要取消，防止内存泄漏
    }
  ...
  private fun getLineHeight(): Float {
    val now = CourseTimeLineImpl.getNowTime()
    ...
    return when {
      ...
      now <= 24 * 60 -> { // 22:30 - 24:00 晚上最后一节课下课到凌晨
        ...
        val end = 24 * 60
        val multiple = (now - start) / (end - start).toFloat()
        val lessonH = course.getHeight() - course.getLessonEndHeight(12)
