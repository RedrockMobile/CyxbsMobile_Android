...
class AskPostedFm : BaseRVFragment<AskPosted>() {
    ...
    override fun bindDataHolder(holder: androidx.recyclerview.widget.RecyclerView.ViewHolder, position: Int, data: AskPosted) {
        ...
        if (data.type == "已解决") {
            ...
            context?.let {
                holder.itemView.mine_ask_posted_tv_disappear_at.textColor = ContextCompat.getColor(it, R.color.common_mine_about_text_color_blue)
            }
        } else {
            ...
            context?.let {
                holder.itemView.mine_ask_posted_tv_disappear_at.textColor = ContextCompat.getColor(it, R.color.common_mine_list_item_tv_disappear_at)
            }
        }
