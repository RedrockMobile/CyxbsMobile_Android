...
class VolunteerAffairAdapter(private var dataList: List<VolunteerAffair>, private val onItemClick: (VolunteerAffair) -> Unit) : RecyclerView.Adapter<VolunteerAffairAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: VolunteerAffairAdapter.ViewHolder, position: Int) {
        holder.itemView.apply {
            findViewById<TextView>(R.id.tv_volunteer_affair_title).text = dataList[position].name
            findViewById<TextView>(R.id.tv_volunteer_affair_reward).text = context.getString(R.string.volunteer_value, dataList[position].hour)
            findViewById<TextView>(R.id.tv_volunteer_affair_start_time).text = context.getString(R.string.volunteer_server_time, stamp2Date(dataList[position].date))
            findViewById<TextView>(R.id.tv_volunteer_affair_sign_up_end).apply {
