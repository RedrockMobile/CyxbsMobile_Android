...
class CircleSquareAdapter(
    val viewModel: CircleSquareViewModel,
    private val onItemClickEvent: (Topic, View, Int) -> Unit
) : BaseRvAdapter<Topic>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder<Topic> =
        CircleSquareViewHolder(parent)


    class CircleSquareViewHolder(parent: ViewGroup) :
        BaseViewHolder<Topic>(parent, R.layout.qa_recycler_item_circle_square) {
        ...
        override fun refresh(data: Topic?) {
            ...
            if (data?._isFollow != null) {
                if (data._isFollow == 1) {
                    ...
                    itemView.btn_circle_square_concern.background =
    ...
    override fun onBindViewHolder(holder: BaseViewHolder<Topic>, position: Int) {
        ...
        holder.itemView.btn_circle_square_concern.setOnClickListener {
            if (dataList[position]._isFollow == 1) {
                //已经关注的状态点击是取消关注
                viewModel.followTopic(
                    dataList[position].topicName,
                    dataList[position]._isFollow == 1
                )
                holder.itemView.btn_circle_square_concern.background =
                    context.getDrawable(R.drawable.qa_shape_send_dynamic_btn_blue_background)
