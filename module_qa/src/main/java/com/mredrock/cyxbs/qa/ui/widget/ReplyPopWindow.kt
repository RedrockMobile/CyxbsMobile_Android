...
object ReplyPopWindow {
    ...
    fun with(context: Context) {
        ...
        mainView = WeakReference(
            LayoutInflater.from(context).inflate(R.layout.qa_popwindow_reply, null, false)
        )
    ...
    fun setReplyName(name: String, content: String) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
            ...
            val s =
                "回复 <font color=\"#556C89\">@${name}</font> : ${content.take(6) + if (content.length > 6) "..." else ""}"
            mainView?.get()?.findViewById<TextView>(R.id.qa_tv_reply_popwindow_name)?.text =
    ...
    fun show(view: View, alignMode: AlignMode, offsetYReverse: Int) {
        ...
        // 显示弹窗
        replyPopWindow?.get()?.showAsDropDown(
            view,
            offsetX,
            -(offsetYReverse + View.MeasureSpec.getSize(mainView?.get()!!.measuredHeight) + view.height),
            Gravity.START
        )
    }

    fun setOnClickEvent(callback: () -> Unit) {
        mainView?.get()?.findViewById<ImageView>(R.id.qa_iv_reply_popwindow_dismiss)
