...
public class LikeView extends View implements Checkable {
     ...
     */
    public static final float RADIUS_INNER_SHAPE_SCALE = 6;
    ...
    private void drawInnerShape(Canvas canvas, int radius, boolean isChecked) {
        if (isHasIcon()){
            ...
            int max = Math.max(icon.getIntrinsicWidth(), icon.getIntrinsicHeight());
            float ratio = (float) max / radius;
            int widthRadius = (int) (icon.getIntrinsicWidth() / ratio);
            int heightRadius = (int) (icon.getIntrinsicHeight() / ratio);
            icon.setBounds(-widthRadius,-heightRadius,widthRadius,heightRadius);
            icon.draw(canvas);
        }else {
    ...
    private class LvAnimatorUpdateListener implements AnimatorUpdateListener {
        ...
        public void onAnimationUpdate(ValueAnimator animation) {
            ...
            if (animatedValue == 0) {
                ...
                mCurrentPercent = Math.min(1f - percent + 0.2f, 1f);//用于计算圆环宽度，最小0.2，与动画进度负相关
                mCurrentRadius = (int) (2 * mRadius * percent);
                if (animatorArgb != null && animatorArgb.isRunning()) {
                    mCurrentColor = (int) animatorArgb.getAnimatedValue();
                }
