  ...
  Box(
    modifier = modifier
      .combinedClickable(
        ...
        onClick = {
          if (!manualCheckInProgress.value) {
    ...
    Row(modifier = Modifier.padding(bottom = 8.dp)) {
      Text(
        ...
        modifier = Modifier.clickableNoIndicator {
          ILegalNoticeService::class.implOrNull()?.openUserAgreementScreen()
            ?: toast("当前应用未实现「用户协议」功能")
        }
      )
      Text(
        text = "&",
      ...
      Text(
        ...
        modifier = Modifier.clickableSingle {
          ILegalNoticeService::class.implOrNull()?.openPrivacyPolicyScreen()
            ?: toast("当前应用未实现「隐私政策」功能")
        }
      )
    }
    Text(
