...
> 当前合同不再使用跨资源批次；每个 Category、Schedule、OccurrenceOverride 操作独立返回结果。

当前字段合并直接由 typed resource 的 `AtomicField<T>` 表达：

- `data` 是完整原子值；
- `modifiedAt` 是该原子的客户端修改时刻；
- Schedule 的 title、description、categoryId、timing、recurrence、reminder、todoState、linkedToCourse 分别独立；其中 categoryId.data=null 表示未分组，reminder.data=null 表示不提醒；
- OccurrenceOverride 的 status、timing、title、description、categoryId、reminder 分别独立；
...
| `Schedule.reminder` | 不提醒 |
| `Schedule.todoState` | 不属于清单；仅事务允许，TODO 必须为 `OPEN` 或 `COMPLETED` |

其他 nullable 字段表示“该联合分支不使用此成员”，请求中应省略，而不是显式发送 `null`：

- `Timing.startAt/endAt/dueAt/date` 由 `kind` 决定：`TIMED` 使用 start/end，`DEADLINE` 使用 due，`ALL_DAY` 使用单个 UTC 午夜 date，`UNSCHEDULED` 全部省略；
...
空集合和空字符串不是 null：`weekdays/monthDays/months` 即使为空也发送 `[]`，允许为空的 description 发送 `""`。客户端网络 JSON 使用 `explicitNulls=true` 与 `encodeDefaults=false`，因此上述 required nullable 原子会编码 `data:null`，带默认 null 的可选成员会被省略；服务端严格解码规则必须与此表同步。
