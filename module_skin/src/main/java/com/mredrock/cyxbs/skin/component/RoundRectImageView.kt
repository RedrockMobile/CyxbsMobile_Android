...
class RoundRectImageView : AppCompatImageView {
     ...
     * 这两个都是画圆的半径
     */
    private var roundWidth = 32
    private var roundHeight = 32
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        if (attrs != null) {
            val a = context.obtainStyledAttributes(attrs, R.styleable.SkinRoundAngleImageView)
            roundWidth = a.getDimensionPixelSize(R.styleable.SkinRoundAngleImageView_skin_roundWidth, roundWidth)
            roundHeight = a.getDimensionPixelSize(R.styleable.SkinRoundAngleImageView_skin_roundHeight, roundHeight)
        } else {
